import { readFile } from "fs/promises";
import { join } from "path";
const source = await readFile(join(import.meta.dirname, "ziggy.wasm"));
let mem;
const module = await WebAssembly.instantiate(source, {
    dbg: {
        print: console.log,
        printMem: (ptr, len, text) => console.log(ptr, len, text
            ? new TextDecoder().decode(new Uint8Array(mem, ptr, len))
            : new Uint8Array(mem, ptr, len)),
    },
});
const wasm = module.instance.exports;
mem = wasm.memory;
const writeString = (str) => {
    const strPtr = wasm.allocString(str.length + 1);
    const strMem = new Uint8Array(wasm.memory.buffer, strPtr, str.length + 1);
    new TextEncoder().encodeInto(str + "\0", strMem);
    return strPtr;
};
// To be kept in sync with zig
var Operation;
(function (Operation) {
    Operation[Operation["map"] = 0] = "map";
    Operation[Operation["array"] = 1] = "array";
    Operation[Operation["tag"] = 2] = "tag";
    Operation[Operation["bytes"] = 3] = "bytes";
    Operation[Operation["integer"] = 4] = "integer";
    Operation[Operation["float"] = 5] = "float";
    Operation[Operation["bool"] = 6] = "bool";
    Operation[Operation["null"] = 7] = "null";
})(Operation || (Operation = {}));
const readSlice = (slicePtr) => {
    const [ptr, len] = new Uint32Array(wasm.memory.buffer, slicePtr, 2);
    return { ptr, len };
};
const sliceToString = (slice) => new TextDecoder().decode(new Uint8Array(wasm.memory.buffer, slice.ptr, slice.len));
const constructFromOps = (ops, i, options) => {
    var _a;
    switch (ops[i.val++]) {
        case Operation.map: {
            const map = {};
            const len = ops[i.val++];
            for (let field = 0; field < len; field++) {
                const fieldName = sliceToString(readSlice(ops[i.val++]));
                const val = constructFromOps(ops, i, options);
                map[fieldName] = val;
            }
            return map;
        }
        case Operation.array: {
            const arr = [];
            const len = ops[i.val++];
            for (let field = 0; field < len; field++) {
                const val = constructFromOps(ops, i, options);
                arr.push(val);
            }
            return arr;
        }
        case Operation.tag: {
            const name = sliceToString(readSlice(ops[i.val++]));
            const bytes = constructFromOps(ops, i, options);
            if ((_a = options === null || options === void 0 ? void 0 : options.literals) === null || _a === void 0 ? void 0 : _a[name]) {
                return options.literals[name](bytes);
            }
            return bytes;
        }
        case Operation.bytes: {
            const bytes = sliceToString(readSlice(ops[i.val++]));
            return bytes;
        }
        case Operation.integer: {
            const l = ops[i.val++];
            const h = ops[i.val++];
            const translator = new Uint32Array([l, h]).buffer;
            return new BigInt64Array(translator, 0, 1)[0];
        }
        case Operation.float: {
            const l = ops[i.val++];
            const h = ops[i.val++];
            const translator = new Uint32Array([l, h]).buffer;
            return new Float64Array(translator, 0, 1)[0];
        }
        case Operation.bool: {
            return !!ops[i.val++];
        }
        case Operation.null: {
            return null;
        }
    }
};
export const parse = (source, options = {}) => {
    const sourcePtr = writeString(source);
    const opsSlicePtr = wasm.parse(sourcePtr, source.length);
    const opsSlice = readSlice(opsSlicePtr);
    const ops = new Uint32Array(wasm.memory.buffer, opsSlice.ptr, opsSlice.len);
    const result = constructFromOps(ops, { val: 0 }, options);
    wasm.parseFree();
    wasm.freeString(sourcePtr);
    return result;
};
